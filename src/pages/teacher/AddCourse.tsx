...
export const TeacherAddCourse: React.FC = () => {
  const navigate = useNavigate();
  const { user, profile } = useAuth();
  const { settings } = useSettings();
  const isSingleMode = settings?.platformMode === 'single';
  ...
  React.useEffect(() => {
    ...
    if (profile?.defaultCommission !== undefined) {
      setFormData((prev) => ({ ...prev, commissionPercentage: profile.defaultCommission }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const courseData = {
        ...
        status: (profile?.role === 'admin' || isSingleMode) ? 'approved' : 'pending',
        teacherId: user.uid,
        teacherName: profile?.displayName || user.displayName || user.email?.split('@')[0] || 'مدرس',
        teacherPhotoURL: profile?.imageUrl || profile?.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile?.displayName || user.displayName || user.email?.split('@')[0] || 'مدرس')}&background=0a1220&color=2563eb`,
